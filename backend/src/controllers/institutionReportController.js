...
export const getInstitutionOverallReport = asyncHandler(async (req, res) => {
  ...
  const { test_id, range, from, to } = req.query;
  ...
  // 1. Fetch institution profile & licence info
  const instRes = await query(`SELECT id, name, code, total_licenses FROM institutions WHERE id = $1`, [instId]);
  const instInfo = instRes.rows[0] || { id: instId, name: 'Institution', total_licenses: 50 };

  // 2. Fetch student breakdown (total, active, inactive)
  ...
  const studentStats = studentStatsRes.rows[0] || { total_enrolled: 0, active_students: 0, inactive_students: 0 };
...
export const getInstitutionRankingsReport = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const instId = validateInstitutionAccess(req, id);
  const { batch_id, batchId, search, sortBy = 'rank', sortOrder = 'ASC', page = 1, limit = 25 } = req.query;
...
export const getInstitutionTrendsReport = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const instId = validateInstitutionAccess(req, id);
  const { interval = 'week', batch_id } = req.query;
  ...
  let trends = result.rows.map((r, index) => ({
    test_id: r.test_id,
...
export const getImprovementAnalyticsReport = asyncHandler(async (req, res) => {
  ...
  let improvements = result.rows.map(r => {
