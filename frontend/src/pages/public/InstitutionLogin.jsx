          ...
          <div className="w-full max-w-[400px] rounded-[20px] border border-[#38BDF8]/25 bg-gradient-to-b from-[#0F213D]/95 via-[#0B1A32]/98 to-[#071224]/98 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.45)] text-white p-5 sm:p-6 space-y-4 relative overflow-hidden">
            ...
            {activeSession ? (
              ...
              <>
                ...
                {error && (
                  <div className="rounded-xl border border-rose-500/30 bg-rose-500/15 p-3 text-xs font-semibold text-rose-200 text-center animate-in fade-in">
                    {error}
                  </div>
                )}

                <form onSubmit={onSubmit} autoComplete="off" className="space-y-3.5">
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider text-slate-200 mb-1" htmlFor="inst-id">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full rounded-xl bg-gradient-to-r from-[#0284C7] via-[#2563EB] to-[#7C3AED] py-3 text-xs sm:text-sm font-bold text-white shadow-[0_6px_16px_rgba(37,99,235,0.20)] hover:-translate-y-[1.5px] hover:shadow-[0_8px_25px_rgba(37,99,235,0.30)] transition cursor-pointer disabled:opacity-50"
                  >
                    {submitting ? (
