...
export default function AietsCalendarPage() {
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto pb-16">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">
            ...
            <span>/</span>
            <span className="text-blue-600 dark:text-cyan-400 font-extrabold">AIETS Calendar</span>
          </div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Plan your preparation, track upcoming assessments and access results and solutions.
          </p>
        </div>
        ...
        <div className="relative shrink-0" ref={programDropdownRef}>
          <button
            ...
            className="flex items-center gap-2.5 bg-white dark:bg-[#071126] px-4 py-2.5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs text-xs font-extrabold text-slate-900 dark:text-white hover:border-blue-500/50 transition cursor-pointer"
          >
          ...
          {programDropdownOpen && (
            <div className="absolute right-0 z-50 mt-2 w-72 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] p-2 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
              <div className="px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500">Enrolled Programs</div>
              {programs.map((p) => (
                <button
                  key={p.id}
                  type="button"
                  ...
                  className={`flex w-full items-center justify-between gap-2.5 rounded-xl px-3 py-2.5 text-xs font-bold transition cursor-pointer ${
                ...
                >
                  ...
                  <span className="px-2 py-0.5 rounded text-[10px] font-black uppercase bg-blue-500/10 text-blue-600 dark:text-blue-300 border border-blue-500/20">
                    {p.badge}
                  </span>
                </button>
              ))}
      ...
      {/* 2. PREMIUM OVERVIEW GRID (SPOTLIGHT + SUMMARY CARDS) */}
      <div className="grid lg:grid-cols-12 gap-6 items-stretch">
        {/* Spotlight Assessment Card (7 cols) */}
        <div className="lg:col-span-7 rounded-3xl border border-blue-200/80 dark:border-blue-500/30 bg-gradient-to-br from-blue-50/90 via-indigo-50/80 to-blue-100/70 dark:from-[#06142e] dark:via-[#091b3e] dark:to-[#071126] p-6 text-slate-900 dark:text-white shadow-xs relative overflow-hidden flex flex-col justify-between">
          <div className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full bg-blue-400/20 dark:bg-cyan-400/15 blur-3xl" />

          <div className="space-y-4 relative z-10">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-blue-600/10 border border-blue-500/30 text-blue-700 dark:bg-cyan-500/20 dark:border-cyan-400/30 dark:text-cyan-300 backdrop-blur-md">
                ⭐ Next Assessment Spotlight
              </span>
              {/* Unified Single Badge */}
              <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-600/10 border border-emerald-500/30 text-emerald-700 dark:bg-emerald-500/20 dark:border-emerald-400/30 dark:text-emerald-300 flex items-center gap-1">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                Phase 1: <strong className="text-blue-700 dark:text-cyan-300">Concept Building</strong> • Scheduled Date: {nextAssessment?.formattedDate}
              </p>
              ...
              <div className="mt-2 inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-blue-600/10 border border-blue-500/20 text-blue-800 dark:text-cyan-300 font-mono font-bold text-xs">
                ...
                <span>{countdownDays} days until {nextAssessment?.name}</span>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-2.5 pt-1">
              <div className="p-3 rounded-xl bg-white/90 dark:bg-slate-900/70 border border-blue-200/60 dark:border-slate-800 text-xs shadow-2xs">
                <span className="text-slate-500 dark:text-slate-400 block text-[10px] font-semibold">Time</span>
                <span className="font-extrabold text-slate-900 dark:text-white">9:00 AM IST</span>
              </div>
              <div className="p-3 rounded-xl bg-white/90 dark:bg-slate-900/70 border border-blue-200/60 dark:border-slate-800 text-xs shadow-2xs">
                <span className="text-slate-500 dark:text-slate-400 block text-[10px] font-semibold">Duration</span>
                <span className="font-extrabold text-slate-900 dark:text-white">3 hours</span>
              </div>
              <div className="p-3 rounded-xl bg-white/90 dark:bg-slate-900/70 border border-blue-200/60 dark:border-slate-800 text-xs shadow-2xs">
                <span className="text-slate-500 dark:text-slate-400 block text-[10px] font-semibold">Marks</span>
                <span className="font-extrabold text-slate-900 dark:text-white">720 marks</span>
              </div>
            </div>
          </div>

          <div className="pt-5 border-t border-blue-200/80 dark:border-slate-800/80 flex items-center justify-between gap-4 relative z-10">
            <span className="text-xs text-slate-600 dark:text-slate-400 font-mono">
              Schedule Window: <strong className="text-blue-700 dark:text-cyan-300 font-semibold">Oct 4, 2026</strong>
            </span>
            <button
              type="button"
              onClick={() => setActiveTest(nextAssessment)}
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs shadow-md shadow-blue-500/25 transition cursor-pointer"
            >
              View Full Details →
            </button>
          </div>
        ...
        {/* 4 Summary Cards (5 cols) with Top Semantic Accent Borders */}
        <div className="lg:col-span-5 grid grid-cols-2 gap-4">
          <div className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 border-t-4 border-t-blue-500 bg-white dark:bg-[#071126] shadow-xs flex flex-col justify-between hover:translate-y-[-2px] transition">
            <div className="flex items-center justify-between">
              <span className="text-xs font-extrabold uppercase text-slate-500 dark:text-slate-400">Total Tests</span>
          ...
          <div className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 border-t-4 border-t-emerald-500 bg-white dark:bg-[#071126] shadow-xs flex flex-col justify-between hover:translate-y-[-2px] transition">
            <div className="flex items-center justify-between">
