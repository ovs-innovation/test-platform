    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className={`rounded-3xl border p-6 backdrop-blur-xl shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
        ...
        <div>
          <h2 className={`text-lg sm:text-xl font-extrabold flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
            ...
            <span>Academic Batches ({batches.length})</span>
          </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 px-4 py-2 text-xs font-bold text-white shadow-md hover:scale-105 transition cursor-pointer shrink-0"
          >
      ...
      {filteredBatches.length > 0 ? (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-lg font-extrabold text-white mt-1.5">{batch.batch_name || batch.name}</h3>
        ...
        <div className={`rounded-3xl border p-12 text-center space-y-3 ${
          ...
          <h3 className="text-lg font-extrabold text-white">No batches created yet</h3>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`w-full max-w-md rounded-3xl border shadow-2xl p-6 space-y-5 ${
            ...
            <h3 className="text-lg font-extrabold text-white">{editingBatch ? 'Edit Batch' : 'Create New Academic Batch'}</h3>

            {error && <p className="text-xs font-bold text-rose-400">{error}</p>}

            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block font-semibold uppercase text-slate-300 mb-1">Batch Name *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full py-2.5 px-3 rounded-xl border border-slate-800 bg-slate-950 text-white focus:outline-none focus:border-purple-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold uppercase text-slate-300 mb-1">Target Exam</label>
                  <select
                ...
                <div>
                  <label className="block font-semibold uppercase text-slate-300 mb-1">Class Level</label>
                  <select
              ...
              <div>
                <label className="block font-semibold uppercase text-slate-300 mb-1">Faculty / Coordinator</label>
                <input
                  type="text"
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2.5 rounded-xl bg-purple-600 font-bold text-xs text-white hover:bg-purple-500"
                >
